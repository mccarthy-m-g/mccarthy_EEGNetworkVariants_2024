...
Oscillation-based functional connectivity was estimated between all pairs of EEG channels within each recording for all five frequency bands using measures of phase and amplitude coupling. Phase and amplitude coupling are thought to reflect different but related neurophysiological functions and it is common to use both measures so they can be compared [@engeletal_IntrinsicCouplingModes_2013; @sadaghianiwirsich_IntrinsicConnectomeOrganization_2020]. Specifically, phase coupling is thought to regulate the integration and flow of cognitive contents on fast timescales, and amplitude coupling is thought to coordinate changes in (co)activation of brain areas on slow timescales [@engeletal_IntrinsicCouplingModes_2013]. We selected measures of phase and amplitude coupling that account for the effects of field spread and volume conduction in the head---which can cause multiple electrodes to record activity from the same underlying neural source [@nunezsrinivasan_ElectricFieldsBrain_2006; @schoffelengross_SourceConnectivityAnalysis_2009]---to reduce the possibility of spurious coupling between channels from influencing our functional connectivity estimates. The resulting functional connectivity estimates from each recording were collected into ten symmetrical $64 \times 64$ matrices called *functional connectomes* (one functional connectome for each frequency band and method), which reflected electrophysiological cortical functional networks derived from averaged oscillatory coupling between each pair of channels. These functional connectomes were the basic unit of observation for subsequent analyses.
...
The average amount of phase coupling between pairs of sensors was estimated within each recording using the phase lag index [@stametal_PhaseLagIndex_2007]. The phase lag index estimates the ..., given by [@vincketal_ImprovedIndexPhasesynchronization_2011]:

$$
\mathit{PLI} = \vert E \langle \mathrm{sign}( \mathrm{Im}(S_{xy}) ) \rangle \vert,
$$

where $S_{xy}$ is the cross spectral density between two sensors, $\mathrm{sign}(\cdot)$ is the signum function, ... and $E$ is the expected value operator.

The phase lag index takes values between 0 and 1. A value of zero indicates either no phase coupling or phase coupling with a phase angle difference centred around 0 mod $\pi$. A value of 1 indicates perfect phase locking with a nonzero phase angle difference. 

We first estimated the the phase lag index between two channels at several frequency bins within each epoch, then we averaged the phase lag index at each frequency bin across epochs, and finally we averaged again across frequency bins to get the final phase lag index estimate. The details of this procedure are described below.
...
First we determined the frequency bins at which the phase lag index between two channels would be estimated by calculating the Discrete Fourier Transform sample frequencies for a 1000 sample window (corresponding to a 5 second epoch) and a sample space of 0.005 (the inverse of the sampling rate). This resulted in 501 frequency bins spaced at roughly 0.2 Hz intervals between zero and 100 Hz. Then for each frequency band we simply selected the frequency bins that were greater than or equal to the minimum frequency and less than or equal to the maximum frequency of each frequency band. 

The tapered spectra were combined, then we estimated the power spectral density at each frequency bin using the combined spectra

Then the cross spectral density between each pair of channels was estimated at each at each frequency bin

Then the *signed* phase lag index was estimated using the cross spectral density at each frequency bin

Then the absolute average of the signed phase lag index was calculated across epochs at each frequency bin

Then the value for each frequency bin was averaged to get the final score

<!-- I think the issue here is that this is technically calculating the phase lag index over trials like Cohen talks about, except instead of doing it at each sample across the epochs, it's treating each epoch as a sample -->


<!-- TODO I could also modify the envelope_correlation function to compute PLI, since the Hilbert transform gives phase too -->

TODO: I believe this method actually is okay, the limitation is that each epoch is treated as "stationary"

Say that we picked PLI because it's commonly used in the literature (maybe look in the cohen book, if he says it's common then you can cite the book)



The power spectral densities were estimated using the multitaper spectral estimation [@babadibrown_ReviewMultitaperSpectral_2014; @thomson_SpectrumEstimationHarmonic_1982] with digital prolate spheroidal sequence (DPSS) [@slepianpollak_ProlateSpheroidalWave_1961] windows.

"These measures are not to be interpreted over time. Each Epoch passed into the dataset is interpreted as an independent sample of the same connectivity structure. Within each Epoch, it is assumed that the spectral measure is stationary."




Cross spectral density is estimated within each epoch at each frequency using multitapers. Right now we’re using a default of 7 tapers.

PLI is estimated within each epoch at each frequency using the cross spectral density

PLI is averaged across epochs at each frequency

The averaged PLI_average is averaged across frequencies to get the final value
...
amplitude coupling between pairs of sensors was estimated using the leakage controlled amplitude envelope correlation [@brookesetal_MeasuringFunctionalConnectivity_2011; @brookesetal_MeasuringFunctionalConnectivity_2012]

pairwise correlations are calculated in each epoch, and then averaged across epochs to get the final estimate

Orthogonalization is done both ways, then the average is taken to get the corr value. As seen in Hipp et al [@hippetal_LargescaleCorticalCorrelation_2012]
...
where $\mathbf{S} = \mathbf{XX}^\mathsf{T}$ and $\mathbf{T} = \mathbf{YY}^\mathsf{T}$. The RV coefficient can thought of as a generalization of the squared Pearson correlation coefficient between two variables to the case of two sets of variables, where two sets of variables are correlated if the relative position (in Hilbert space) of the observations in one set is similar to the relative position of the observations in the other set [@josseholmes_MeasuringMultivariateAssociation_2016; @mayeretal_ExploratoryAnalysisMultiple_2011]. However, as can be seen in Equation \@ref(eq:rv) above, unlike the Pearson correlation coefficient, the mean of the observations in a set is not subtracted from its observations when estimating the RV coefficient [@abdiherve_RVCoefficientCongruence_2007].

After estimating the RV coefficient between all pairs of functional connectomes within each frequency band, we assessed how functional connectome similarity varied between and within individuals, sessions, and states using data visualization and statistical modelling.

#### Visualizing functional connectome similarity

we collected the RV coefficient estimates into a similarity matrix
...
The precision parameter was assumed to be constant across observations.

constant phi

binary indicators

Our estimand from the model was between and within contrasts
